...
public class ItemSubCollar extends ItemAmuletVis implements ICosmeticAttachable {
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean par4) {
        ...
        list.add(EnumChatFormatting.GOLD + StatCollector.translateToLocal("item.capacity.text") + " "
                + this.getMaxVis(stack) / 100);
        if (stack.hasTagCompound()) {
            ...
            while (count.hasNext()) {
                ...
                if (stack.stackTagCompound.hasKey(aspect.getTag())) {
                    String amount = this.myFormatter.format(
                            (double) ((float) stack.stackTagCompound.getInteger(aspect.getTag()) / 100.0F));
                    list.add("\u00a7" + aspect.getChatcolor() + aspect.getName() + EnumChatFormatting.RESET + " x "
                            + amount);
                }
            }
            if (stack.stackTagCompound.hasKey("owner"))
                list.add(StatCollector.translateToLocal("tooltip.collar.owner") + " "
                        + stack.stackTagCompound.getString("owner"));
    ...
    public boolean itemInteractionForEntity(ItemStack itemstack, EntityPlayer player, EntityLivingBase entity) {
        ...
        if (entity instanceof EntityPlayer) {
            ...
            if (baubles.getStackInSlot(0) == null) {
                ...
                sub.addChatMessage(new ChatComponentText(StatCollector.translateToLocal("message.collar.placescollar")
                        .replace("%s", player.getDisplayName())));
                player.addChatMessage(
                        new ChatComponentText(StatCollector.translateToLocal("message.collar.youplacecollar")
                                .replace("%s", sub.getDisplayName())));
                return true;
            } else
                player.addChatMessage(
                        new ChatComponentText(StatCollector.translateToLocal("message.collar.alreadywearing")
                                .replace("%s", sub.getDisplayName())));
        }
        return false;
    }
