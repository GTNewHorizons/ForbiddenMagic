...
public class ItemFocusBlink extends ItemFocusBasic {
    ...
    FocusUpgradeType hellfire = ForbiddenItems.getUpgrade(
            Config.hellfireUpgradeID,
            new ResourceLocation("forbidden", "textures/misc/hellfire.png"),
            "forbidden.upgrade.hellfire.name",
            "forbidden.upgrade.hellfire.text",
            (new AspectList()).add(DarkAspects.NETHER, 1));
    FocusUpgradeType pandemonium = ForbiddenItems.getUpgrade(
            Config.pandemoniumUpgradeID,
            new ResourceLocation("forbidden", "textures/misc/pandemonium.png"),
            "forbidden.upgrade.pandemonium.name",
            "forbidden.upgrade.pandemonium.text",
    ...
    public ItemStack onFocusRightClick(ItemStack itemstack, World world, EntityPlayer player,
        ...
        MovingObjectPosition mop = BlockUtils.getTargetBlock(
                ...
                (player.prevRotationPitch + (player.rotationPitch - player.prevRotationPitch)),
                false,
                128.0);
                ...
                if (this.isUpgradedWith(wand.getFocusItem(itemstack), hellfire)) {
                    ...
                    for (EntityLivingBase entity : entities) {
                        ...
                        for (int k = 0; k < 8; ++k) {
                            world.spawnParticle(
                                    "flame",
                                    entity.posX + (world.rand.nextDouble() - 0.5D) * (double) entity.width,
                                    entity.posY + world.rand.nextDouble() * (double) entity.height - 0.25D,
                                    entity.posZ + (world.rand.nextDouble() - 0.5D) * (double) entity.width,
                                    0,
                                    0,
                                    0);
                        }
