...
public class WrathSpawnerLogic {
    ...
    public void setMobID(String par1Str) {
        this.mobID = par1Str;
        if (Config.spawnerMobs.containsKey(mobID)) aspect = Config.spawnerMobs.get(mobID);
        else aspect = Aspect.GREED;
    }
    ...
    public void updateSpawner() {
        ...
        if (this.getSpawnerWorld().isRemote && (fuel > 0 || Config.wrathCost <= 0)) {
            ...
            if (fuel <= 0 && Config.wrathCost > 0) return;
            ...
            for (int i = 0; i < this.spawnCount && (fuel > 0 || Config.wrathCost <= 0); ++i) {
                ...
                // AxisAlignedBB.getAABBPool().getAABB((double) this.getSpawnerX(), (double) this.getSpawnerY(),
                // (double) this.getSpawnerZ(), (double) (this.getSpawnerX() + 1), (double) (this.getSpawnerY() + 1),
                // (double) (this.getSpawnerZ() + 1)).expand((double) (this.spawnRange * 2), 4.0D, (double)
                // (this.spawnRange * 2))).size();
                ...
                // entityliving.getCanSpawnHere())
                {
                    ...
                    this.updateDelay();
                } else if (entityliving != null) entityliving.isDead = true;
            }
        }
    }
