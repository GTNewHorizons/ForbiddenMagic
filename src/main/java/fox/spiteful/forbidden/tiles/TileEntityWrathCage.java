...
public class TileEntityWrathCage extends TileEntity implements IAspectContainer, IEssentiaTransport {
    ...
    public void checkAspect() {
        if (aspect != spawnLogic.getAspect()) special = 0;
        aspect = spawnLogic.getAspect();
    }
    ...
    public AspectList getAspects() {
        ...
        if (Config.wrathCost > 0 && spawnLogic.isMobSet()) {
            if (mode == 0) list.add(aspect, special);
            else if (mode == 1) list.add(DarkAspects.WRATH, wrath);
            else list.add(DarkAspects.SLOTH, sloth);
        }
        return list;
    }
    ...
    public boolean doesContainerAccept(Aspect tag) {
        if (Config.wrathCost <= 0) return false;
        else return (tag == DarkAspects.WRATH || tag == DarkAspects.SLOTH || tag == aspect);
    }
    ...
    public int addToContainer(Aspect tag, int amount) {
        if (Config.wrathCost <= 0) return amount;
    ...
    public boolean doesContainerContainAmount(Aspect tag, int amount) {
        if (tag == DarkAspects.WRATH) return wrath >= amount;
        else if (tag == DarkAspects.SLOTH) return sloth >= amount;
        else if (tag == aspect) return special >= amount;
        else return false;
    }
    ...
    public int containerContains(Aspect tag) {
        if (tag == DarkAspects.WRATH) return wrath;
        else if (tag == DarkAspects.SLOTH) return sloth;
        else if (tag == aspect) return special;
        else return 0;
    }
    ...
    public boolean isConnectable(ForgeDirection face) {
        if (Config.wrathCost > 0) return true;
        else return false;
    }
    ...
    public Aspect getSuctionType(ForgeDirection face) {
        ...
        if (mode == 0) return aspect;
        else if (mode == 1) return DarkAspects.WRATH;
        else return DarkAspects.SLOTH;
    }
    ...
    public int getSuctionAmount(ForgeDirection face) {
        if (Config.wrathCost > 0) return 256;
        else return 0;
    }
    ...
    public int getEssentiaAmount(ForgeDirection face) {
        if (mode == 0) return special;
        else if (mode == 1) return wrath;
        else return sloth;
    }
